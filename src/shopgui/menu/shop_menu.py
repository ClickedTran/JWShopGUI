...
class ShopMenu:
    ...
    def openMenu(self, player: Player):
        ...
        for shop_name, shop_data in all_shop.items():
            ...
            icon_meta.lore = [str(LanguageManager.get_translate("menu.category.lore.open"))]
        ...
        def on_confirm(player: Player, slots: int, item_clicked: ItemStack, inv: UIInventory) -> None:
            ...
            if display_name == LanguageManager.get_translate("menu.confirm.add_count"):
                ...
                def has_money(player_has_money: bool):
                    if player_has_money:
                        ...
                        if total_price > 0:
                            ...
                            self.manager.get_money(player, "coins", lambda new_balance: add_item(new_balance))
                    else:
                ...
                if total_price > 0:
                    ...
                    self.manager.get_money(player, "coins", lambda new_balance: remove_item(new_balance))

        menu.set_listener(on_confirm)
        menu.send_to(player)

    def openCustomBuy(self, player: Player, item: ItemStack):
        menu = Menu(MenuType.DOUBLE_CHEST, LanguageManager.get_translate("menu.category.stack.name"))     
        for i in range(0, 54):
        ...
        for slot in [13, 22, 31]:
            irron_bar = ItemStack("minecraft:iron_bars")
            irron_bar_meta = irron_bar.item_meta
            irron_bar_meta.display_name = LanguageManager.get_translate("menu.category.stack.barrier")
            irron_bar.set_item_meta(irron_bar_meta)
