## Generator
ShopGUI plugin for EndStone software, adapted from ShopGUI in PocketMine-MP software.
Both are owned by ClickedTran (ClickedTran_VN).

## Install
- Step 1: Download and unzip file
- Step 2: Run Command Propmt or Power Shell as Administrator
- Step 3: Go to the file containing your server system
```python
venv\Scripts\activate
```
- Step 4: Go to file path
```python
cd ShopGUI_Endstone
```
- Step 5: Run command and wait:
```python
pip install -e .
```
- Step 6: Go back your server system
- Step 7: Open your server and play!
